...
public class TaxiAllocator {
    public static int minNumTaxis(int[][] requests) {
        ...
        PriorityQueue<Integer> availableTaxis = new PriorityQueue<>();

        for (int[] request : requests) {
            int pickup = request[0];
            int dropoff = request[1];

            if (!availableTaxis.isEmpty() && availableTaxis.peek() <= pickup) {
                availableTaxis.poll();
            }

            availableTaxis.add(dropoff + 1);
        }
