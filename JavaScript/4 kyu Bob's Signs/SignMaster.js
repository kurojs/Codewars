...
Bob is running a business that creates signs for people. He can charge much less than his competitors
because he charges by letter instead by the entire sign. He can take a sign and change a few letters
to make a new sign much more cheaply than a competitor can make a sign from scratch.

The only problem is Bob is not very good at pricing these changes. He wants to be able to look at a sign
and a customer's request and quickly be able to give the customer an estimate for the total cost.
Task
...
estimatePrice takes two strings. The first string is the old sign of the customer and the second is their request.
...
SignMaster.prototype.changePrices = function (prices) {
  if (prices.hasOwnProperty("add")) this.addCost = prices.add;
  if (prices.hasOwnProperty("rem")) this.remCost = prices.rem;
};
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      if (oldSign[i - 1] === newSign[j - 1]) {
        ...
        dp[i][j] = Math.min(
          dp[i - 1][j] + this.remCost,
          dp[i][j - 1] + this.addCost,
        );
      }
    }
  }
