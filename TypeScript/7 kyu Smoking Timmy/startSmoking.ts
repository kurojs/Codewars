/*
Timothy (age: 16) really likes to smoke. Unfortunately, he is too young to buy his own cigarettes and 
that's why he has to be extremely efficient in smoking.
...
Please note that Timothy never starts smoking cigarettes that aren't "full size" 
so the amount of smoked cigarettes is always an integer.
*/

// My solution

export function startSmoking(bars: number, boxes: number): number {
  let cigarettes = bars * 10 * 18 + boxes * 18;
  let total = cigarettes;

  while (cigarettes >= 5) {
    let newCigs = Math.floor(cigarettes / 5);
    total += newCigs;
    cigarettes = newCigs + (cigarettes % 5);
  }
